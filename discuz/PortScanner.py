# -*- coding: utf-8 -*-
import threading, socket, sys, cmd, os, Queue, time
    ...
    def Ping(self, Port):
        ...
        if lock.acquire():
            try:
                ...
                print '[+]IP:%s    %d/tcp    open    %s' % (self.IP, Port, serv)
            except:
                print '[+]IP:%s    %d/tcp    open    %s' % (self.IP, Port, 'unknown')
            lock.release()
        return True
...
class Shell(cmd.Cmd):
    ...
    def __init__(self):
        ...
        self.prompt = "PortScan >>"
        self.intro = "Starting PortScanner at %s" % time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
